...
export const getHotel = defineAction({
  ...
  handler: async (number) => {
    const hotelCode = number.number;

    const apiKey = import.meta.env.SECRET_BEDSONLINE_API_KEY;
    const secret = import.meta.env.SECRET_BEDSONLINE_API_SECRET;
    const timestamp = Math.floor(Date.now() / 1000);
    const BEDSONLINE_API_URL = `https://api.test.hotelbeds.com/hotel-content-api/1.0/hotels/${hotelCode}/details?language=ENG&useSecondaryLanguage=False`;
